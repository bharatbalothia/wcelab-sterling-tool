...
//			Collections.sort(apiList);
			...
			Document lastInput = null;
			
			for (ChainedApi chainedApi : apiList) {
				
				Document inputDocument = XmlUtil.getDocument(output);
				
				if (lastInput != null) {
					inputDocument.getDocumentElement()
						.appendChild(inputDocument.createElement("NZApiLastInput"))
						.appendChild(inputDocument.adoptNode(lastInput.getDocumentElement()));
				}
				
				if (SkunkSetting.getInstance().VERBOSE_MODE) {
					System.out.println("\n*******************************************************************");
					System.out.println(  "***** API execution input pre XSLT    *****************************");
					System.out.println(XmlUtil.getXmlString(inputDocument));
					System.out.println("\n*******************************************************************\n");
				}
				...
				output = ApiExecutor.executeApi(
						interopUrlProperty.getSelectedItem(),
						isHttpClientProperty.get(), userIDProperty.get(),
						passwordProperty.get(), programID, chainedApi.name, chainedApi.flowName, inputDocument, chainedApi.inputTransformation, 
						chainedApi.outputTemplate, chainedApi.outputTransformation, verboseMode);
				
				lastInput = inputDocument;
			}
